...
### 虚拟环境配置
**指令：** 所有涉及代码执行、依赖项检查或环境交互的任务，**必须优先在 `E:\AI\cursor\starone\.venv` 虚拟环境中执行**。

### Claude 执行环境要求
**重要：** Claude 在执行所有 Python 相关任务时，必须：
1. **优先使用 Git Bash 环境**
2. **执行前激活虚拟环境**：`source /E/AI/cursor/starone/.venv/Scripts/activate`
...
```json
{
    "python.defaultInterpreterPath": "E:\\AI\\cursor\\starone\\.venv\\Scripts\\python.exe",
...
# 2. 验证环境（推荐）
which python
# 应显示：/e/AI/cursor/starone/.venv/Scripts/python

...
class CVELocalDatabase:
    ...
    def save_cve(self, cve_data, solution_brief="", solution_detailed=""):
        """
        保存单个CVE数据到数据库
    ...
    def search_offline(self, search_type="all", keyword="", severity_filter=None, date_range=None):
        """
        离线搜索CVE数据
    ...
    def save_search_history(self, search_type, keyword, result_count):
        """保存搜索历史"""
        try:
            cursor = self.conn.cursor()
            cursor.execute("""
    ...
    def export_to_json(self, output_file="cve_export.json"):
        """
        导出数据库到JSON文件

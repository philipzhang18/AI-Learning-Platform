...
#### 6. 月度趋势迷你图（P2-12）
选中任意一行微码后，下方 Canvas 自动绘制近 12 个月命中柱状图，附带文字标签：
...
| ↓ 冷却 | 最近 3 月 < 前 3 月 × 0.5 |
| → 平稳 | 介于两者之间 |
| ↑ 新发 | 前 3 月为零，最近 3 月有命中 |
| 无活动 | 12 月内无命中 |

通过 `MicrocodeRiskAssessor.monthly_hits(key, months=12)` 取序列，便于运维识别"最近是否在升级窗口期"。
...
`risk/dsa_cve_backfill.py` 用于检查 DSA→CVE 引用反查率，并按需通过 NVD 回填缺失：

```bash
# 仅诊断，不写库
python risk/dsa_cve_backfill.py --diagnose-only

# 拉前 100 条缺失（带 NVD API Key 速度提升 ~10x）
python risk/dsa_cve_backfill.py --limit 100 --api-key YOUR_NVD_KEY
```

输出：本地 `cves` 表对 DSA 引用 CVE 的覆盖率（当前 ~48.3%，缺口集中 2018-2023）、按年份分布、修复 SQL。

#### 共享 DSA 加载器（P3-13）

`risk/_dsa_base.py` 抽取了三个预测器（产品线 / 版本 / 微码）共用的低层工具，避免逻辑漂移：

| 函数 | 用途 |
|---|---|
| `parse_advisory_data(json_str)` | 安全解析 `dell_advisories.data` JSON |
| `parse_cve_ids(text)` | 从 `cve_ids` 列拆出规范 CVE-ID 列表 |
| `affected_text_of(d, include_version_range)` | 拼装 `name + model[+ version_range]` 文本，供产品线分类 |
| `severity_text_of(d)` | 取 `severity` 大写文本 |
| `load_cve_score_map(db)` | 一次性预加载 `cves.cvss_score` 内存映射 |
| `fetch_advisory_rows(db)` | 流式迭代 `dell_advisories` 4 列原始数据 |
| `resolve_cvss(cve_ids, score_map, severity)` | 双源 CVSS：NVD 优先，severity 文本兜底 |

#### 嵌套 Markdown 导出（P3-15）

智能预测 Tab → 「导出报告（Markdown）」按钮输出按产品线分组的层次化报告：
...
## 算法说明        ← 仅出现一次
## PowerEdge (服务器)
   <a id="poweredge-服务器"></a>
   ### DSA 概率预测（未来 90 天）
   ### 微码风险 Top 10
## PowerScale / Isilon (NAS)
   ...
## 知识图谱产品风险评分（如有）
```

锚点保留中文，3000 字节内可生成 7 个产品线完整报告。
...
| 版本 | 日期 | 主要变更 |
|------|------|----------|
| v5.6.0 | 2026-05-31 | 新增🕸 知识图谱标签页（NetworkX 内存图，CVE × DSA × 产品 × CWE 四类节点 / 三类关系），支持邻域子图可视化、GraphML/JSON 导出 |
